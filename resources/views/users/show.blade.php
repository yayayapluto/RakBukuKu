...
<body class="font-poppins bg-home">
    ...
    <div class="relative ml-48 mt-24">
        <!-- Lingkaran biru (di bawah kiri) -->
        <div class="absolute w-64 h-64 bg-blue rounded-full z-0 -left-16 -bottom-16"></div>
        
        <!-- Lingkaran merah (di atas kanan, lebih ke kanan dan ke atas) -->
        <div class="absolute w-64 h-64 bg-merah rounded-full z-0 -right-16 -top-16"></div>

        <!-- Kartu (di atas lingkaran) -->
        <div class="relative w-96 h-64 bg-SecondColor rounded-md opacity-65 z-10 p-6"> 
            <img src="{{asset('Storage/logoo.svg')}}" alt="" class="w-12 h-12 opacity-100">
            <p class="pt-5 text-lg font-semibold">{{ $data->id }}</p>
            <p class="text-xs">{{ $data->tanggal_bergabung }}</p>
            <div class="flex justify-between">
                <p class="pt-16 font-semibold">{{ $data->nama }}</p>
                <p class="text-xs pt-16 mt-1">{{ $data->tanggal_bergabung }}</p>
            </div>
        </div>
    </div>
